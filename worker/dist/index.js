...
// src/proxy.ts
function upstreamOrigin(url, env2) {
  if (url.pathname.startsWith("/courses/") || url.hostname.includes("courses.")) {
    return env2.COURSES_ORIGIN;
  }
  return env2.MADRASA_ORIGIN;
}
__name(upstreamOrigin, "upstreamOrigin");
function buildUpstreamUrl(workerUrl, env2) {
  const origin = upstreamOrigin(workerUrl, env2);
  const upstream = new URL(workerUrl.pathname + workerUrl.search, origin);
  return upstream;
}
__name(buildUpstreamUrl, "buildUpstreamUrl");
function rewriteLocationHeader(response, workerOrigin, env2) {
  const location = response.headers.get("Location");
  if (!location)
    return response;
  let rewritten = location.replace(env2.MADRASA_ORIGIN, workerOrigin).replace(env2.COURSES_ORIGIN, workerOrigin);
...
function buildUpstreamHeaders(incoming, upstreamHost) {
...
async function proxyRequest(request, env2) {
  const workerUrl = new URL(request.url);
  const workerOrigin = workerUrl.origin;
  const upstreamUrl = buildUpstreamUrl(workerUrl, env2);
  ...
  const upstreamRequest = new Request(upstreamUrl.toString(), {
    ...
    redirect: "manual"
    // Handle redirects ourselves so we can rewrite Location
  });
  ...
  return rewriteLocationHeader(response, workerOrigin, env2);
}
__name(proxyRequest, "proxyRequest");

// src/transform.ts
var BLOCKED_SCRIPT_SRCS = [
...
__name(ScriptRemover, "ScriptRemover");
var MobileCSSInjector = class {
  element(element) {
    element.append(`<style id="madrasa-mobile">${MOBILE_CSS}</style>`, { html: true });
  }
};
__name(MobileCSSInjector, "MobileCSSInjector");
var ViewportFixer = class {
  element(element) {
...
__name(ViewportFixer, "ViewportFixer");
var ViewportInserter = class {
  element(element) {
    element.prepend(
...
__name(ViewportInserter, "ViewportInserter");
function transformResponse(response) {
  return new HTMLRewriter().on("script", new ScriptRemover()).on("head", new MobileCSSInjector()).on('meta[name="viewport"]', new ViewportFixer()).on("head", new ViewportInserter()).transform(response);
}
__name(transformResponse, "transformResponse");

// src/cache.ts
var CACHE_NAME = "madrasa-proxy-v1";
var LESSON_TTL = 60 * 60;
function isCacheable(url, method) {
  if (method !== "GET")
    return false;
  const path = url.pathname;
  if (path.includes("/login") || path.includes("/register") || path.includes("/password"))
    return false;
...
var src_default = {
  async fetch(request, env2, _ctx) {
    ...
    let response = await proxyRequest(request, env2);
